...
- 无对象存储（S3/R2）配置时，视频生成直接以 `RESULT_PERSIST_FAILED` 失败并给出明确错误，不得回退短期上游 URL；开发/测试用本地模拟存储或测试开关补偿。
- 提示词库封面/预览图保持上游 URL，不转存 S3；媒体持久化只覆盖生成结果（`GenerationImage`/`GeneratedVideo`）。
- 定时同步默认关闭（`PROMPT_SYNC_ENABLED=false`），由部署者显式开启并配置间隔。
